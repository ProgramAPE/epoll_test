...
fn main() -> Result<()> {
    ...
    while handled_events < n_events{
        ...
        poll.poll(&mut events, Some(5000))?;
        if events.is_empty() {
            println!("TIMEOUT (OR SPURIOS EVENT NOTIFICATION)");
            continue;
        }
        println!("handle event {handled_events} start");

        handled_events += handle_events(&events, &mut streams)?;
        println!("handle event {handled_events} finished");
    }

    //std::thread::sleep(std::time::Duration::from_secs(10));

    println!("FINISHED. total handles: {}", handled_events);
    Ok(())
